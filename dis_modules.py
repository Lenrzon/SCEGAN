    ...
    with tf.variable_scope(scope, reuse=reuse):
        def str2float(s):
            if s[0] == '-':
                s = s[1:]
                a = s.split('.')
                return -round((reduce(lambda x, y: x * 10 + y, map(int, a[0])) \
                               + reduce(lambda x, y: x / 10 + y, map(int, a[1][::-1])) / 10), 12)
            else:
                a = s.split('.')
                return round(reduce(lambda x, y: x * 10 + y, map(int, a[0])) \
                             + reduce(lambda x, y: x / 10 + y, map(int, a[1][::-1])) / 10, 12)
...
def multihead_attention(queries,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # global_pooling = tf.reduce_mean(Q, axis=1, keepdims=True)  # (B, 1, num_units)
        # Q += global_pooling  # Broadcast to (B, N, num_units)
...
def cat_embedding(inputs,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        f_transe = open('movielens_attributes/MFGAN_normalized_embeddings.txt', 'r', encoding='UTF-8')
        id2embedding = []
        for line in f_transe:
            line = line.strip().split()
            for i in range(len(line)):
